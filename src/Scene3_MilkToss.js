...
export class Scene3_MilkToss {
    constructor(renderer, onWin) {
        ...
        // Floor
        const floor = new THREE.Mesh(new THREE.PlaneGeometry(50, 50), new THREE.MeshStandardMaterial({ color: 0x224422 }));
        ...
        const cube = new THREE.Mesh(new THREE.BoxGeometry(3, 1, 3), new THREE.MeshStandardMaterial({ color: 0xffffff }));
        ...
        this.showUI();
        // Mouse events
        this.renderer.domElement.addEventListener("mousedown", () => this.startCharging());
        this.renderer.domElement.addEventListener("mouseup", () => this.throwBall());
    }
    // Win conditions
    handleWin() {
        if (this.hasWon)
    ...
    loadModel(path, onLoad) {
        ...
        loader.load(path, (gltf) => onLoad(gltf.scene), undefined, (err) => {
    ...
    loadBottles(basePosition) {
        ...
        pyramidOffsets.forEach((offset, index) => {
            this.loadModel("/assets/models/bottle.glb", (model) => {
                model.position.set(basePosition.x + offset[0], basePosition.y + offset[1], basePosition.z + offset[2]);
    ...
    throwBall() {
        ...
        this.charging = false;
        const ball = new THREE.Mesh(new THREE.SphereGeometry(0.2, 16, 16), new THREE.MeshStandardMaterial({ color: 0xffdd00 }));
        const camDir = new THREE.Vector3();
        this.camera.getWorldDirection(camDir);
        ball.position.copy(this.camera.position).add(camDir.clone().multiplyScalar(1));
        const speed = THREE.MathUtils.lerp(5, 20, this.meterValue);
        ball.userData.velocity = camDir.clone().multiplyScalar(speed);
        this.scene.add(ball);
    ...
    update(delta) {
        ...
        this.balls.forEach((ball) => {
            ...
            this.bottles.forEach((bottle) => {
                ...
                if (!data.broken) {
                    ...
                    if (distance < 0.5) {
                        data.broken = true;
                        data.velocity = new THREE.Vector3((Math.random() - 0.5) * 2, Math.random() * 2 + 1, (Math.random() - 0.5) * 2);
                    }
                }
            });
        ...
        this.bottles.forEach((bottle) => {
            ...
            if (!data.broken && data.supports && data.supports.length > 0) {
                const allBroken = data.supports.every((s) => s.userData.broken);
                if (allBroken) {
                    data.broken = true;
                    data.velocity = new THREE.Vector3((Math.random() - 0.5) * 1.5, Math.random() * 2 + 1, (Math.random() - 0.5) * 1.5);
                }
            }
