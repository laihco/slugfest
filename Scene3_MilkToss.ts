...
export class Scene3_MilkToss {
  ...
  private handleWin() {
    ...
    setTimeout(() => {
      ...
      this.onWin(); // safe now, non-optional
    }, 2000);
  }
  ...
  private handleLose() {
    ...
    setTimeout(() => {
      loseOverlay.remove();
      this.onWin(); // use same callback to return to hub
    }, 2000);
  }
  ...
  update(delta: number) {
    ...
    const allDown = this.bottles.length > 0 &&
      this.bottles.every((bottle) => {
        const data = bottle.userData as BottleData;
        const onGround = bottle.position.y <= 0.11;
        return data.broken && onGround;
      });

    if (!this.gameOver && allDown) {
      this.handleWin();
    }
    ...
    if (
      ...
      if (!this.waitingForResult) {
        ...
        this.outOfBallsTimer = 0;
      } else {
        this.outOfBallsTimer += delta;

        if (this.outOfBallsTimer >= 1.0 && !allDown) {
          this.handleLose();
        }
      }
    } else {
    ...
    if (
      this.gameOver && this.hasWon && this.prizeFox && this.prizeFox.visible
    ) {
